...
const VolunteerDashboard = () => {
  ...
  const myGrades = grades.filter(g => myStudents.some(s => s.id === g.studentId));
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GlassCard className="p-6">
          <h3 className="text-white text-xl font-semibold mb-4">Ações Rápidas</h3>
        ...
        <GlassCard className="p-6">
          <h3 className="text-white text-xl font-semibold mb-4">Meus Alunos</h3>
          <div className="space-y-3">
            {myStudents.map(student => (
              <div key={student.id} className="bg-white/10 p-3 rounded-lg">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-white font-medium">{student.name}</h4>
                    <p className="text-blue-200 text-sm">{student.turma}</p>
                  </div>
      ...
      <GlassCard className="p-6">
        <h3 className="text-white text-xl font-semibold mb-4">Progresso dos Alunos</h3>
        <div className="space-y-4">
          {myStudents.map(student => {
            const studentGrades = grades.filter(g => g.studentId === student.id);
            const avgBefore = studentGrades.reduce((acc, g) => acc + g.gradeBefore, 0) / (studentGrades.length || 1);
            const avgAfter = studentGrades.reduce((acc, g) => acc + g.gradeAfter, 0) / (studentGrades.length || 1);
            ...
            return (
              <div key={student.id} className="bg-white/5 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-white/60 text-sm">Média Antes</p>
                    <p className="text-red-300 text-xl font-bold">{avgBefore.toFixed(1)}</p>
                  </div>
                  <div>
                    <p className="text-white/60 text-sm">Média Atual</p>
                    <p className="text-green-300 text-xl font-bold">{avgAfter.toFixed(1)}</p>
                  </div>
                  <div>
